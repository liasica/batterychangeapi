...
func (*group) Start() error {
    ...
    //             if service.GroupDailyStatService.GenerateWeek(context.TODO(), item.Id, model.BatteryType60) != nil {
    //                 g.Log().Error(fmt.Sprintf("团体 %d 型号 %d 生成失败", item.Id, 60))
    //             }
    //             if service.GroupDailyStatService.GenerateWeek(context.TODO(), item.Id, model.BatteryType72) != nil {
