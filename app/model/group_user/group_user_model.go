...
// arModel is a active record design model for table group_user operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of group_user.
	Table = "group_user"
	// Model is the model object of group_user.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table group_user.
	Columns = struct {
		Id         string //           
        GroupId    string // 团队ID    
        UserId     string // 用户ID    
