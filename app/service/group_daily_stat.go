...
// RiderBizNew 用户新领取
func (s *groupDailyStatService) RiderBizNew(ctx context.Context, groupId uint, batteryType string, userId uint64) error {
...
// RiderBizExit 用户退租
func (s *groupDailyStatService) RiderBizExit(ctx context.Context, groupId uint, batteryType string, userId uint64) error {
    var max model.GroupDailyStat
    if err := dao.GroupDailyStat.Ctx(ctx).
        Where(dao.GroupDailyStat.Columns.GroupId, groupId).
        Where(dao.GroupDailyStat.Columns.BatteryType, batteryType).
...
func (s *groupDailyStatService) GenerateWeek(ctx context.Context, groupId uint, batteryType string) error {
    max := struct {
        GroupId uint
        UserIds []uint64
        Date    uint
...
func (s *groupDailyStatService) StatDateRange(ctx context.Context, groupIds []uint, startDate, endDate *gtime.Time) (stat map[uint]*groupDailyStatDateRangeItem, err error) {
    ...
    for _, row := range res {
        if row.BatteryType == model.BatteryType60 {
            if row.IsArrears == 0 {
