...
func (*signService) UserLatestDoneDetail(ctx context.Context, userId, comboOrderId uint64, groupId uint) (sign *model.Sign, err error) {
    sign = &model.Sign{}
    err = dao.Sign.Ctx(ctx).
        Where(dao.Sign.Columns.UserId, userId).
        Where(dao.Sign.Columns.ComboOrderId, comboOrderId).
